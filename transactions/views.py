...
from transactions.models import TopUp,Package
    ...
    def post(request):
        with transaction.atomic():
            try:
                ...
                tell_num = request.data.get('tell_num')
                tell_charger = request.data.get('tell_charger')
                package_type = request.data.get('package_code')
                packageLog = Package.create(
                    amount=amount,
                    broker=broker,
                    tell_num=tell_num,
                    tell_charger=tell_charger,
            ...
            call_response_type, call_response_description = MCI().package_call_sale(
                packageLog.tell_num,
                packageLog.tell_charger,
                packageLog.amount,
                packageLog.package_type,
            )
            success = packageLog.after_call(call_response_type, call_response_description)
            if success:
                data = {
                    "message": "success: request successfully submitted",
                    "message_fa": "موفقیت: درخواست با موفقیت ثبت شد",
                    "code": 0,
                    "provider_id": packageLog.provider_id
                }
                return Response(data, status=status.HTTP_200_OK)
            else:
                data = {
                    ...
                    "message_fa": "خطا: خطا در ثبت درخواست",
                    "code": -11,
                    "response_type": packageLog.call_response_type,
                    "response_description": packageLog.call_response_description
                }
                return Response(data, status=status.HTTP_200_OK)
